...
signal basic_content_finished_loading(content)
...
func _ready()-> void:
	...
	basic_content_finished_loading.connect(on_basic_content_finished_loading)
	pass
func load_level_basic(content_path:String) -> void:
	_transition = "basic"
...
func monitor_load_status()->void:
	...
	match load_status:
		...
		ResourceLoader.THREAD_LOAD_LOADED:
			...
			if _transition =="basic":
				basic_content_finished_loading.emit(ResourceLoader.load_threaded_get(_content_path).instantiate())
			else:
...
func on_content_failed_to_load(path:String) ->void :
	printerr("error: Failed load resources:'%s'" % [path])
func on_content_invalid(path:String)->void:
	printerr("error: Cannot load resources:'%s'" % [path])
...
func on_basic_content_finished_loading(content)->void:
	var outgoing_scene = get_tree().current_scene
